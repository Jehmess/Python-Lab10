...
with open('girlsName.txt', 'r') as in_file:
    names = (line.strip() for line in in_file)
    lines = (line.split(',') for line in names if line)
    with open('lab10Girls.csv', 'w') as out_file:
        write = csv.writer(out_file)
        write.writerow(('First Name', 'Count'))
        write.writerows(lines)

with open('boysName.txt', 'r') as in_file:
    names = (line.strip() for line in in_file)
    lines = (line.split(',') for line in names if line)
    with open('lab10Boys.csv', 'w') as out_file:
        write = csv.writer(out_file)
        write.writerow(('First Name', 'Count'))
        write.writerows(lines)
